...
class Wire:
    def __init__(self, bus_id, sda, scl, freq=400000):
        self._i2c   = I2C(bus_id, sda=Pin(sda), scl=Pin(scl), freq=freq)
        adresses    = self.scan()
        self.address = None if len(adresses) == 0 else adresses[0]
        print(f'(wire) sda={sda} scl={scl} - devices found: {[hex(address) for address in adresses]}')
        sleep(1.0)
